...
print("\n-----------------------------------")
print("Guild Wars System Loading:\n")
...
function Newguildgift(eventId, guild, leader, name) -- idea provided by creativextent . wrote by BlackWolf
	...
	SendWorldMessage(GWCOMM[Server].color_13.."The Guild "..name.." lead by "..leader:GetName().." has entered existence!|r")
end
...
GGW[player:GetAccountId()] = {
	...
	if(player:GetGuildName())then
		...
		player:SendBroadcastMessage(GWCOMM[Server].color_14.."Use '/guild "..GWCOMM[Server].commands.."' for a list of Guild Wars commands.")	
	end
end
...
-- ****************************************************
	...
	if(player:IsInGuild()==true)then
		...
		if(ChatCache[1]==GWCOMM[Guildname].commands)then
			...
			player:SendBroadcastMessage(GWCOMM[Guildname].color_8.."*  -Guild Warz Commands:-  *");
			player:SendBroadcastMessage(GWCOMM[Guildname].color_9.."*********************************************");
...
--			
			if(ChatCache[1] == "reload")then
				...
				if(ChatCache[2] == GWCOMM[Server].table)then
					GWtable = {};
					LoadGWtable();
					player:SendBroadcastMessage("|cff00cc00Guild Wars Tables Reloaded.|r");
				return false;
				end
...
local function pig_payz(eventid, timer, cycles, player)
	...
	if(player:IsInGuild() == true)then
		...
		player:SendBroadcastMessage("Requin shouts:|cffff0000Join a guild to earn hourly rewards from Guild Wars.|r");
	end
end
...
print ("Pig Pay version: "..pigpayz_version);
	...
	if(startpos == 1) then
		...
		if(player:IsInGuild()==true)then
			...
			if(GWARZ[Loc]==nil)then
				player:SendBroadcastMessage("Sin says:|cffcc0000error.... teleport entry doesn't exsist.|r");
			else
...
local locid = GetLocationId(player);
	...
	if(player:IsInGuild()==false)then
		...
		if(GWCOMM[GWARZ[locid].guild_name].guild_invite == 1)then
			player:SendBroadcastMessage("|cff00cc00Guild Warz System:|r")
			player:SendBroadcastMessage("|cff00cc00"..GWARZ[locid].guild_name.." own\'s this location "..player:GetName()..".|r");
			player:SendBroadcastMessage("|cff00cc00Join a Guild to participate in the Guild Wars System.|r");
			Gwarz_Guild_Flag_Hello(1, player, go)
			return false;
		else
			player:SendBroadcastMessage("|cff00cc00Guild Wars System:|r");
			player:SendBroadcastMessage("|cff00cc00"..GWARZ[locid].guild_name.." own\'s this location "..player:GetName()..".|r");
			player:SendBroadcastMessage("|cff00cc00Join a Guild to participate in the Guild Wars System.|r");

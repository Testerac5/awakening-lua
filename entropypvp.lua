...
local function EntropyPvP(event, pKiller, pKilled)
	...
	if (playerdeath==true and check_safe == false) then
		...
		if ((pKiller:GetLevel()-pKilled:GetLevel())<=leveldiff) then
			...
			FullLootContainer:RegisterEvent(Remove_FullLootContainer, 300000, 0, true)				--Register the Remove/Despawn event to the Sack of Belongings
...
local function Init_FullLootFrame(event, player, object)
	local test = item_table[object:GetGUIDLow()]
	FullLootFrame:Clear()
	...
	for k, v in pairs(item_table[object:GetGUIDLow()]) do
		...
		if (FullLoot_Button~=nil) then
			...
			local function AddItem(player)
				...
				player:SendBroadcastMessage("You looted "..v[1])
